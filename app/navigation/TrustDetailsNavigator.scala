...
class TrustDetailsNavigator @Inject() (appConfig: AppConfig) extends Navigator {
  ...
  private def conditionalNavigation(): PartialFunction[Page, UserAnswers => Call] = {
    ...
    case GovernedByUkLawPage            =>
      yesNoNav(_, GovernedByUkLawPage, AdministeredInUkController.onPageLoad(), GoverningCountryController.onPageLoad())
    case AdministeredInUkPage           =>
      ua =>
        yesNoNav(
          ua,
    ...
    case EfrbsYesNoPage                 =>
      yesNoNav(_, EfrbsYesNoPage, EfrbsStartDateController.onPageLoad(), firstQuestionAfterTrustTypeQuestions)
    case WhereTrusteesBasedPage         => navigateAwayFromWhereTrusteesBasedQuestion
    case SettlorsUkBasedPage            =>
      yesNoNav(
        _,
        SettlorsUkBasedPage,
        CreatedUnderScotsLawController.onPageLoad(),
        BusinessRelationshipInUkController.onPageLoad()
      )
    case PreviouslyResidentOffshorePage =>
      ua =>
        yesNoNav(
          ua,
          PreviouslyResidentOffshorePage,
          PreviouslyResidentOffshoreCountryController.onPageLoad(),
          navigateToSchedule3aExemptQuestion(ua)
        )
    case SettlorBenefitsFromAssetsPage  =>
      ua =>
        yesNoNav(
          ua,
          SettlorBenefitsFromAssetsPage,
          navigateToSchedule3aExemptQuestion(ua),
          ForPurposeOfSection218Controller.onPageLoad()
        )
    case ForPurposeOfSection218Page     =>
      ua =>
        yesNoNav(
          ua,
          ForPurposeOfSection218Page,
          AgentCreatedTrustController.onPageLoad(),
          navigateToSchedule3aExemptQuestion(ua)
        )
  }
  ...
  private def navigateToSchedule3aExemptQuestion(ua: UserAnswers): Call =
    if (
      appConfig.schedule3aExemptEnabled && (ua
        .get(Schedule3aExemptYesNoPage)
        .isDefined || ua.migratingFromNonTaxableToTaxable)
    ) {
